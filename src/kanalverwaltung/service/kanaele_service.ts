...
import {Kanal} from '../model/kanal';
import {log, isBlank, isEmpty} from '../../shared/shared';

// API-Dokumentation zu mongoose:
// http://mongoosejs.com/docs/api.html

export default class KanaeleService implements IKanaeleService {
    ...
    findById(id: string): Promise<MDocument> {
        ...
        return <Promise<MDocument>>Kanal.findById(id);
    }
    ...
    find(query?: any): Promise<Array<MDocument>> {
        ...
        if (isBlank(query) || Object.keys(query).length === 0) {
            const tmpQuery: Query<MDocument> = <Query<MDocument>>Kanal.find();
        ...
        return <Promise<Array<MDocument>>>(Kanal.find(query));
        // Buch.findOne(query), falls das Suchkriterium eindeutig ist
    }

    @log
    save(buch: MDocument): Promise<MDocument> {
    ...
    update(kanal: MDocument): Promise<{}> {
        ...
        return <Promise<{}>>Kanal.findByIdAndUpdate(<string>kanal._id, kanal);
    ...
    remove(id: string): Promise<{}> {
        ...
        return <Promise<{}>>Kanal.findByIdAndRemove(id);
    ...
    toString(): string { return 'KanaeleService'; }
}
